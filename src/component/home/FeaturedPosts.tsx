...
export function FeaturedPostsSection() {
  return (
    <section className="py-8 md:py-12 bg-background" dir="rtl">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <a href={featuredPost.url} className="block relative w-full h-[500px] rounded-2xl overflow-hidden group shadow-lg">
              ...
              <div className="absolute bottom-0 right-0 p-6 md:p-8 text-white z-20 space-y-4">
                ...
                <div>
                   <Badge className="bg-primary mb-2">{featuredPost.category}</Badge>
                   <h2 className="text-2xl md:text-4xl font-extrabold transition-colors duration-300 group-hover:text-primary">
                     {featuredPost.title}
                   </h2>
                </div>
          ...
          <div className="dark:bg-transparent rounded-2xl justify-between flex flex-col gap-4">
            {sidePosts.map((post, index) => (
              <a 
                key={index} 
                href={post.url} 
                className="flex items-center gap-4 bg-white dark:bg-gray-800 p-3 rounded-xl transition-all duration-300 group"
              >
                    <div className="flex-shrink-0 w-22 h-22 rounded-lg overflow-hidden">
                  <Image 
                    src={post.imageUrl} 
                    alt={post.title}
                    width={96} 
                ...
                <div className="flex-grow space-y-1">
                  <h3 className="font-bold text-md leading-tight group-hover:text-primary transition-colors">
                    {post.title}
                  </h3>
